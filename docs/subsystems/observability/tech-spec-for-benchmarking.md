...
This spec is narrower than [white-paper-on-observability-layer.md](white-paper-on-observability-layer.md). The white paper explains the broader observability architecture. This document defines the strict bench-only contract.
...
- observability store schema and storage internals beyond the two bench tables (covered by [tech-spec-for-observability-store-schema.md](tech-spec-for-observability-store-schema.md) and [tech-spec-for-observability-store-implementation.md](tech-spec-for-observability-store-implementation.md))
...
[bench-route-selection.ts](../../../src/subsystems/observability/bench-route-selection.ts) normalizes the operator's selection into a non-empty `routeNames: string[]`. Issues are reported as one of:
...
[bench-path-mode.ts](../../../src/subsystems/observability/bench-path-mode.ts) defines exactly three values: `"gateway"`, `"direct"`, `"both"`. `isBenchPathModeValue()` is the single guard. Any other value is a usage error.
...
Defined in [bench-limits.ts](../../../src/subsystems/observability/bench-limits.ts):
...
The `surface: "benchmark"` marker is load-bearing: trace listings must filter benchmark rows out of normal traffic queries unless they explicitly opt in. See [field-matrix-for-observability-store.md](field-matrix-for-observability-store.md) for the canonical interpretation of the `surface` field.
...
Defined in [schema.ts](../../../src/subsystems/observability/schema.ts) and [benchmarks.ts](../../../src/subsystems/observability/benchmarks.ts):
...
Indexes (defined in [schema.ts](../../../src/subsystems/observability/schema.ts)):
...
- [bench-route-selection.test.ts](../../../src/subsystems/observability/bench-route-selection.test.ts)
- [bench-execution-validation.test.ts](../../../src/subsystems/observability/bench-execution-validation.test.ts)
- [bench-path-mode.test.ts](../../../src/subsystems/observability/bench-path-mode.test.ts)
