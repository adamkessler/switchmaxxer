...
- [src/subsystems/observability/optimize-report-builder.ts](../../../../src/subsystems/observability/optimize-report-builder.ts)
  is the public facade for the Ostrich optimize report builder, which owns
  shared candidate selection, cost/latency ranking, report persistence, and
  report rehydration
...
| Schema / CRUD | [src/subsystems/observability/benchmarks.ts](../../../../src/subsystems/observability/benchmarks.ts) public facade over `ostrich/benchmark/benchmarks.ts` | [src/subsystems/observability/optimizations.ts](../../../../src/subsystems/observability/optimizations.ts) public facade over `ostrich/optimization/optimizations.ts` |
| Child rows | `benchmark_samples` (one per sample) | none — ranked candidates serialized into `result_json` |
| Built by | [`bench-runner.ts`](../../../../src/subsystems/observability/bench-runner.ts) public facade over `ostrich/benchmark/bench-runner.ts` | [`buildOptimizationRunRecord`](../../../../src/subsystems/observability/optimize-report-builder.ts) public facade over `ostrich/optimization/optimize-report-builder.ts` |
...
### 6.1.3 Apply/Restore Idempotency

CLI and MCP apply/restore requests enter the mutation service through a planned
optimize idempotency bridge. The bridge records one
`optimize_mutation_idempotency` row for each JSON-safe mutation command before
the catalog write begins. Retries with the same idempotency key and stable
command digest replay the previously accepted result instead of creating a
second config mutation event or a second successful Ledger row.

The idempotency row is internal execution state, not the operator-facing audit
record. The Control Plane Audit Ledger still records attempts and outcomes, and
`config_mutation_events` still records only effective committed provider
mutations. The replay row exists to protect command dispatch across CLI/MCP
retries, process interruptions, and future Osprey/Owl transport boundaries.

Reload and verification are intentionally completed by the caller after the
mutation service writes the catalog. For changed non-dry-run mutations with
post-actions, the bridge returns a deferred completion handle; the caller runs
reload and/or verification, then stores the final replay payload. That means a
later retry sees the same action id, warning payload, and post-action status
without repeating the catalog mutation.

Digest comparison is based on stable intent rather than the whole command
object. It includes the operation, run or restore selector, target route,
dry-run/reload/verification flags, source surface, actor/session context, and
other policy-significant fields. It excludes the idempotency key itself and
volatile catalog context so a successful apply can be retried after the current
catalog already reflects the applied provider.

### 6.1.4 Optimize-History Cleanup
...
- `optimize-report-builder.ts` public facade over `ostrich/optimization/optimize-report-builder.ts`
- `optimize-orchestrator.ts` public facade over `ostrich/optimization/optimize-orchestrator.ts`
- `optimize-ledger-views.ts` public facade over `ostrich/optimization/optimize-ledger-views.ts`
