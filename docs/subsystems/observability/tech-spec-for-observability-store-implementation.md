...
- enable foreign keys
- enable WAL
- apply the current schema statements
- seed store metadata

Schema posture:

- the repo treats the schema as one live snapshot
- incompatible schema versions fail closed at open time instead of being reset automatically
- the code keeps bootstrap and metadata simple instead of carrying a separate upgrade system

Future technical debt:

- add an explicit `switchmaxxer observability migrate` command before the observability store is treated as a durable operator-facing upgrade surface
- that future command should own all intentional schema-upgrade behavior instead of startup bootstrap deleting or silently rewriting local data
