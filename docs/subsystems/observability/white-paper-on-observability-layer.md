...
| --- | --- | --- |
| `switchmaxxer prune` / `smx prune` | whole-store prune | Observability records eligible under the configured retention policy, including traces, benchmark history, optimize history, facts, config mutation events, and snapshots |
