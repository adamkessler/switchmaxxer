...
- [tech-spec-for-gateway-observation-mapping.md](../observability/contracts/tech-spec-for-gateway-observation-mapping.md)
...
request has cleared ingress validation. In product and network-architecture
terms, this is reverse-proxy behavior for LLM API traffic: the client calls
Switchmaxxer as the API origin, and the gateway resolves the configured route
to an upstream provider. It is not a general forward proxy that tunnels
caller-selected arbitrary destinations.

It is called by two gateway entry points:

- [`proxyChatCompletion`](../../../src/subsystems/hot-path/manatee/proxy/proxy.ts) — OpenAI listener surface
- [`proxyAnthropicMessage`](../../../src/subsystems/hot-path/manatee/proxy/proxy.ts) — Anthropic listener surface

And by the CLI route-test runtime via [`runRouteTestsDetailed`](../../../src/subsystems/hot-path/manatee/proxy/proxy.ts).
...
[src/subsystems/hot-path/manatee/proxy/proxy.ts](../../../src/subsystems/hot-path/manatee/proxy/proxy.ts): the CLI route-test surface
...
- [src/subsystems/hot-path/manatee/proxy/proxy.test.ts](../../../src/subsystems/hot-path/manatee/proxy/proxy.test.ts) — focused unit
  tests for translation helpers, error classification, abort-reason handling,
  and buffered-limit enforcement
- [src/subsystems/hot-path/manatee/proxy/proxy-runtime.test.ts](../../../src/subsystems/hot-path/manatee/proxy/proxy-runtime.test.ts)
...
- `src/subsystems/hot-path/manatee/proxy/proxy.ts` stays a public barrel. If it starts accreting
  implementation detail, that detail belongs in an implementation module.
- `src/subsystems/hot-path/manatee/proxy/proxy-core.ts` stays the shared request pipeline.
