...
async function withExampleConfigEnv<T>(fn: () => Promise<T>): Promise<T> {
  ...
  const tempDir = mkdtempSync(path.join(tmpdir(), "switchmaxxer-mcp-empty-secrets-"));
  ...
  previous.set(SWITCHMAXXER_SECRETS_PATH_ENV, process.env[SWITCHMAXXER_SECRETS_PATH_ENV]);
...
void test("MCP optimize_apply and optimize_restore mutate a route provider with managed restore snapshots", async () => {
  ...
  try {
    ...
    const applyIdempotencyRow = applyLedgerDb
      .prepare(
        `
      ...
      .get(`apply:${runId}:gpt-4o-mini:false:true:true`) as {
        operation?: string;
        status?: string;
      } | undefined;
    applyLedgerDb.close();
    ...
    const replayApplyPayload = await callToolWithSession("optimize_apply", {
      run_id: runId,
      route_id: "gpt-4o-mini",
      reload: true,
      verify: true
    }, configPath, sessionContext, runtimeDeps);
    const replayApplyData = replayApplyPayload["data"] as Record<string, unknown>;
    assert.equal(replayApplyPayload["ok"], true);
    assert.equal(replayApplyData["action_id"], applyActionId);
    assert.equal(routeProvider("gpt-4o-mini"), "openrouter");
    ...
    const restoreIdempotencyRow = restoreLedgerDb
      .prepare(
        `
          SELECT operation, status
          FROM optimize_mutation_idempotency
      ...
      .get(`restore:action:${applyActionId}:false:true:true`) as {
        operation?: string;
        status?: string;
      } | undefined;
    ...
    const replayRestorePayload = await callToolWithSession("optimize_restore", {
      action_id: applyActionId,
      reload: true,
      verify: true
    }, configPath, sessionContext, runtimeDeps);
    const replayRestoreData = replayRestorePayload["data"] as Record<string, unknown>;
    assert.equal(replayRestorePayload["ok"], true);
    assert.equal(replayRestoreData["action_id"], restoreData["action_id"]);
    assert.equal(routeProvider("gpt-4o-mini"), "openai_direct");
    assert.deepEqual(postActionCalls, [
      { kind: "reload", operation: "apply" },
      { kind: "verify", operation: "apply", routeId: "gpt-4o-mini" },
