...
void test("CLI command-family help surfaces include docs footers", async () => {
  const helpCases: Array<{ argv: string[]; docsPath: string }> = [
    ...
    { argv: ["bench", "--help"], docsPath: "docs/subsystems/observability/contracts/tech-spec-for-benchmarking.md" },
    { argv: ["optimize", "--help"], docsPath: "docs/subsystems/observability/contracts/tech-spec-for-optimize-command.md" },
    { argv: ["ledger", "--help"], docsPath: "docs/subsystems/observability/contracts/tech-spec-for-control-plane-audit-ledger.md" },
    { argv: ["trace", "--help"], docsPath: "docs/subsystems/observability/contracts/tech-spec-for-observation-semantics.md" },
    {
      argv: ["prune", "--help"],
      docsPath: "docs/subsystems/observability/store/tech-spec-for-observability-store-implementation.md"
    }
  ];
...
void test("runCli optimize apply mutates only the target route provider to the persisted winner", async () => {
  ...
  try {
    ...
    const applyIdempotencyRow = applySnapshotDb
      .prepare(
        `
          SELECT operation, status
          FROM optimize_mutation_idempotency
          WHERE idempotency_key = ?
        `
      )
      .get(`apply:${optimizePayload.data.run.run_id}:gpt-4o-mini:false:false:false`) as {
        operation?: string;
        status?: string;
      } | undefined;
    ...
    assert.equal(applyIdempotencyRow?.operation, "optimizeMutations.apply");
    assert.equal(applyIdempotencyRow?.status, "completed");
    ...
    const applyReplayBeforeDb = new DatabaseSync(dbPath);
    const applyReplayBefore = applyReplayBeforeDb
      .prepare(
        `
          SELECT
            (SELECT COUNT(*) FROM config_mutation_events WHERE operation = 'optimize_apply') AS mutation_count,
            (SELECT COUNT(*) FROM control_plane_action_events WHERE operation = 'optimize_apply' AND status = 'succeeded') AS ledger_count
        `
      )
      .get() as { mutation_count?: number; ledger_count?: number } | undefined;
    applyReplayBeforeDb.close();
    const replayApplyResult = await runWithCapturedIo([
      "optimize",
      "apply",
      optimizePayload.data.run.run_id,
      "--route",
    ...
    assert.equal(replayApplyResult.result, 0);
    assert.equal(replayApplyResult.stderr, "");
    const replayApplyPayload = parseCliEnvelope(replayApplyResult.stdout) as {
      data: {
        action_id: string | null;
        changed: boolean;
      };
    };
    assert.equal(replayApplyPayload.data.action_id, applyPayload.data.action_id);
    assert.equal(replayApplyPayload.data.changed, true);
    assert.equal(
      ((readJsonForTests(catalogPath)["routes"] as Record<string, Record<string, unknown>>)["gpt-4o-mini"])?.["service_provider"],
      "openrouter"
    );
    const applyReplayAfterDb = new DatabaseSync(dbPath);
    const applyReplayAfter = applyReplayAfterDb
      .prepare(
        `
          SELECT
            (SELECT COUNT(*) FROM config_mutation_events WHERE operation = 'optimize_apply') AS mutation_count,
      ...
      .get() as { mutation_count?: number; ledger_count?: number } | undefined;
    applyReplayAfterDb.close();
    assert.deepEqual(applyReplayAfter, applyReplayBefore);
    ...
    const restoreIdempotencyRow = restoreSnapshotDb
      .prepare(
        `
          SELECT operation, status
          FROM optimize_mutation_idempotency
      ...
      .get(`restore:action:${applyPayload.data.action_id ?? ""}:false:false:false`) as {
        operation?: string;
        status?: string;
      } | undefined;
    ...
    assert.equal(restoreIdempotencyRow?.operation, "optimizeMutations.restore");
    assert.equal(restoreIdempotencyRow?.status, "completed");
    ...
    const restoreReplayBeforeDb = new DatabaseSync(dbPath);
    const restoreReplayBefore = restoreReplayBeforeDb
      .prepare(
        `
          SELECT
            (SELECT COUNT(*) FROM config_mutation_events WHERE operation = 'optimize_restore') AS mutation_count,
            (SELECT COUNT(*) FROM control_plane_action_events WHERE operation = 'optimize_restore' AND status = 'succeeded') AS ledger_count
        `
      )
      .get() as { mutation_count?: number; ledger_count?: number } | undefined;
    restoreReplayBeforeDb.close();
    const replayRestoreResult = await runWithCapturedIo([
      "optimize",
      "restore",
      applyPayload.data.action_id ?? "",
      "--config",
    ...
    assert.equal(replayRestoreResult.result, 0);
    assert.equal(replayRestoreResult.stderr, "");
    const replayRestorePayload = parseCliEnvelope(replayRestoreResult.stdout) as {
      data: {
        action_id: string | null;
        changed: boolean;
      };
    };
    assert.equal(replayRestorePayload.data.action_id, restorePayload.data.action_id);
    assert.equal(replayRestorePayload.data.changed, true);
    assert.deepEqual(
      (readJsonForTests(catalogPath)["routes"] as Record<string, Record<string, unknown>>)["gpt-4o-mini"],
      originalTargetRoute
    );
    const restoreReplayAfterDb = new DatabaseSync(dbPath);
    const restoreReplayAfter = restoreReplayAfterDb
      .prepare(
        `
          SELECT
            (SELECT COUNT(*) FROM config_mutation_events WHERE operation = 'optimize_restore') AS mutation_count,
      ...
      .get() as { mutation_count?: number; ledger_count?: number } | undefined;
    restoreReplayAfterDb.close();
    assert.deepEqual(restoreReplayAfter, restoreReplayBefore);
