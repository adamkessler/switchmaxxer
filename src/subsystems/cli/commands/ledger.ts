...
import {
  ...
  type ControlPlaneActionEventRecord,
...
interface LedgerObservabilityService {
  controlPlaneActions: {
...
export type LedgerObservabilityHandle = ObservabilityRuntimeHandle & {
  service: LedgerObservabilityService;
};
...
export type LedgerCommandDeps = {
  ...
  openExistingObservabilityService: (dbPath: string) => LedgerObservabilityHandle | null;
...
export function createLedgerCli(deps: LedgerCommandDeps): {
  ...
  function runLedgerList(argv: string[]): number {
    ...
    let handle: LedgerObservabilityHandle | null = null;
    let dbPath = "";
    ...
    try {
      ...
      const events = handle?.service.controlPlaneActions.listEvents(filters) ?? [];
  ...
  function runLedgerShow(ledgerEventId: string, argv: string[]): number {
    ...
    try {
      dbPath = deps.resolveObservabilityStorePath();
      handle = deps.openExistingObservabilityService(dbPath);
      const event = handle?.service.controlPlaneActions.getEvent(ledgerEventId) ?? null;
  ...
  function getHelpText(): string {
    return buildHelpText({
      ...
      docsPath: "docs/subsystems/observability/tech-spec-for-control-plane-audit-ledger.md",
