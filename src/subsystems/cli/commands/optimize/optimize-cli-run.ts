...
function runCostOptimize(deps: OptimizeCliDeps, parsedArgs: OptimizeRunArgs, requestedRoutes: string[] | null): number {
  try {
    ...
    const handle = deps.openObservabilityService(dbPath);
    let persistedReport: OptimizeReportView;
    try {
      persistedReport = persistCostOptimizeReport({
...
async function runLatencyOptimize(
  ...
  let handle: ObservabilityRuntimeHandle | null = null;
  ...
  try {
    ...
    handle = deps.openObservabilityService(dbPath);
    const config = deps.loadConfig(parsedArgs.configPath);
    const runnerResult = await deps.runBenchmarkOperation({
      service: handle.service,
      config,
    ...
    const persistedReport = persistLatencyOptimizeReport({
