...
export function createBenchReadCommands(deps: BenchReadCommandDeps): {
  ...
  async function runBenchList(argv: string[]): Promise<number> {
    ...
    try {
      ...
      const listResult = deps.benchmarkHistory.list({
        dbPath,
        limit: parsedArgs.limit ?? 25
      });
      const runViews = listResult.runs.map(({ run, summary }) => deps.toBenchmarkRunView(run, summary));
      ...
      if (!listResult.storeFound || runViews.length === 0) {
  ...
  async function runBenchShow(runId: string, argv: string[]): Promise<number> {
    ...
    try {
      dbPath = deps.resolveObservabilityStorePath();
      const showResult = deps.benchmarkHistory.show({
        dbPath,
        runId
      });
