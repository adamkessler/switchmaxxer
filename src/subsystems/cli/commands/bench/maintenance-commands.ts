...
import type { ObservabilityBenchmarkHistoryPort } from "../../../observability/observability-module";
...
export function createBenchMaintenanceCommands(deps: BenchMaintenanceCommandDeps) {
  ...
  async function runBenchPrune(argv: string[]): Promise<number> {
    ...
    try {
      ...
      const pruneResult = deps.benchmarkHistory.pruneOlderThan({
        dbPath,
        cutoffIso: cutoffAt
      });
  ...
  async function runBenchDelete(runId: string, argv: string[]): Promise<number> {
    ...
    try {
      dbPath = deps.resolveObservabilityStorePath();
      const deleteResult = deps.benchmarkHistory.deleteRun({
        dbPath,
        runId
      });
      const result = deleteResult.result;

      if (!deleteResult.storeFound || !result || result.benchmark_runs_deleted === 0) {
        if (parsedArgs.json) {
  ...
  async function runBenchClear(argv: string[]): Promise<number> {
    ...
    try {
      dbPath = deps.resolveObservabilityStorePath();
      const clearResult = deps.benchmarkHistory.clear({ dbPath });
