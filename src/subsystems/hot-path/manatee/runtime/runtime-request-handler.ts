...
import type { HotPathConfigSlice } from "../hot-path-config-slice";
...
export function createGatewayRuntimeRequestHandler(
  ...
  const requestHandler: GatewayRuntimeRequestHandler = Object.assign(
    ...
    // Hot-path read-scope enforcement: narrow `runtime.config` from
    // the full `AppConfig` to the curated slice this code may read.
    // See src/subsystems/hot-path/manatee/hot-path-config-slice.ts.
    const config: HotPathConfigSlice = runtime.config;
    ...
    const inboundAuthState = resolveInboundGatewayAuthState(runtime.config);
