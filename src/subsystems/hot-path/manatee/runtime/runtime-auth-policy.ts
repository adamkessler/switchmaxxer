...
export function enforceHealthRequestTrustPolicy(params: {
  ...
  if (
    ...
    emitObservation({
      ...
      outcome: authDecision.status === "blocked" ? "denied_rate_limited" : "denied_misdirected_host",
      backoffApplied: true,
      retryAfterSeconds: authDecision.status === "blocked" ? authDecision.retryAfterSeconds : null,
      statusCode: authDecision.status === "blocked" ? 429 : 421,
...
export function rejectFailedInboundAuth(params: {
  ...
  emitObservation({
    ...
    method: params.method,
    pathname: params.pathname,
    outcome: authDecision.status === "blocked" ? "denied_rate_limited" : "denied_bad_token",
    backoffApplied: true,
    retryAfterSeconds: authDecision.status === "blocked" ? authDecision.retryAfterSeconds : null,
    statusCode: authDecision.status === "blocked" ? 429 : 401,
    reason: null,
    message:
      authDecision.status === "blocked"
        ? "Too many failed authentication attempts."
