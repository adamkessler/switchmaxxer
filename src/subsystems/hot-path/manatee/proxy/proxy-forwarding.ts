...
import { emitLegacyGatewayObservation } from "../observation-emit";
...
import { recordSmxToClientInspectionBody } from "../runtime/invoke-inspection";
...
export function resolveRoute(
  config: Pick<AppConfig, "routes">,
  context: ProxyRequestContext
): RouteConfig | null {
  // Narrowed parameter: route resolution reads only `routes`. The
  // narrower type is structurally compatible with the full AppConfig
  // that callers pass, so this is a one-way tightening with no caller
  // changes required. See
  // src/subsystems/hot-path/manatee/hot-path-config-slice.ts for the
  // broader read-scope contract.
