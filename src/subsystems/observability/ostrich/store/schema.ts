export const OBSERVABILITY_SCHEMA_VERSION = 8;
...
export const CREATE_OPTIMIZE_MUTATION_IDEMPOTENCY_SQL = `
CREATE TABLE IF NOT EXISTS optimize_mutation_idempotency (
  ...
  result_json TEXT NOT NULL,
  error_json TEXT NOT NULL,
  CHECK (operation IN ('optimizeMutations.apply', 'optimizeMutations.restore')),
  CHECK (status IN ('accepted', 'completed', 'failed', 'unknown')),
  FOREIGN KEY (control_plane_action_id) REFERENCES control_plane_action_events(id) ON DELETE SET NULL
);
`;
...
const OBSERVABILITY_INDEX_SQL = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_optimize_mutation_idempotency_action_id ON optimize_mutation_idempotency(control_plane_action_id);`,
  `CREATE INDEX IF NOT EXISTS idx_optimize_mutation_idempotency_status_updated ON optimize_mutation_idempotency(status, updated_at);`,
...
export const OBSERVABILITY_SCHEMA_TABLE_DEFINITIONS = [
  ...
  { name: "optimize_mutation_idempotency", createSql: CREATE_OPTIMIZE_MUTATION_IDEMPOTENCY_SQL },
