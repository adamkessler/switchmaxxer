...
void test("gateway observation worker fatal messages reject pending writes", async () => {
  const runtime = createGatewayObservationRuntimeState();
  const deps = createTestWorkerDeps("unused-worker-script.js");
  const currentGeneration = runtime.worker.getGatewayObservationWorkerGenerationForTests();
  const pendingWrite = runtime.worker.createPendingGatewayObservationWorkerWriteForTests(11, 500);

  runtime.worker.handleGatewayObservationWorkerMessageForTests(
    currentGeneration,
    {
      type: "fatal",
      message: "synthetic worker fatal"
    },
    deps
  );

  await assert.rejects(pendingWrite, /synthetic worker fatal/);
  assert.equal(runtime.worker.getPendingGatewayObservationWorkerWriteCountForTests(), 0);
});

void test("gateway observation worker reports unexpected worker exits", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "switchmaxxer-observability-worker-exit-"));
  const workerScriptPath = path.join(tempDir, "exiting-worker.js");
...
`
  ...
  try {
    await assert.rejects(
      runtime.worker.ensureGatewayObservationWorker(path.join(tempDir, "observability.sqlite"), deps),
      /exited with code 42/
    );

    assert.equal(failures.length, 1);
    assert.match(failures[0]?.message ?? "", /exited with code 42/);
    assert.equal(runtime.worker.getPendingGatewayObservationWorkerWriteCountForTests(), 0);
  } finally {
    await runtime.worker.disposeGatewayObservationWorker(deps);
    rmSync(tempDir, { recursive: true, force: true });
