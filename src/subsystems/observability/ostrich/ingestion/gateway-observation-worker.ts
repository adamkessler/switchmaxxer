...
function refHandle(handle: { ref?: () => void } | null): void {
  handle?.ref?.();
}
...
export function createGatewayObservationWorkerState() {
  ...
  function handleGatewayObservationWorkerMessage(
    ...
    switch (message.type) {
      ...
      case "batch_written": {
        ...
        if (pendingGatewayObservationWorkerWrites.size === 0) {
          unrefHandle(gatewayObservationWorker);
        }
        return;
      }
  ...
  async function writeGatewayObservationBatch(
    ...
    refHandle(gatewayObservationWorker);
